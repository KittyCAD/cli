...
func TestNewCmdConvert(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "input no extension",
			cli:        "./no-extension thing.stl",
			isTTY:      true,
			wants:      Options{},
			wantStdout: "",
			wantStderr: "",
			wantErr:    "input file must have an extension or you must pass the file type with `--from` or `-f`",
		},
		{
			name:       "output no extension",
			cli:        "thing.stl ./no-extension",
			isTTY:      true,
			wants:      Options{},
			wantStdout: "",
			wantStderr: "",
			wantErr:    "output file must have an extension or you must pass the file type with `--to` or `-t`",
		},
		{
			name:       "unsupported input file type",
			cli:        "./no-extension.html thing.stl",
			isTTY:      true,
			wants:      Options{},
			wantStdout: "",
			wantStderr: "",
			wantErr:    "unsupported input file format: `html`",
		},
		{
			name:       "unsupported output file type",
			cli:        "./thing.stl blah.nope",
			isTTY:      true,
			wants:      Options{},
			wantStdout: "",
			wantStderr: "",
			wantErr:    "unsupported output file format: `nope`",
		},
		{
			name:       "unsupported output file format",
			cli:        "./thing.stl --to blah",
			isTTY:      true,
			wants:      Options{},
			wantStdout: "",
			wantStderr: "",
			wantErr:    "unsupported output file format: `blah`",
		},
		{
