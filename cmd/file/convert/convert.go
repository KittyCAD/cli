...
func NewCmdConvert(cli *cli.CLI, runF func(*Options) error) *cobra.Command {
	...
	for _, v := range kittycad.FileSourceFormats {
		validSourceFormats = append(validSourceFormats, string(v))
	}

	validOutputFormats := []string{}
	for _, v := range kittycad.FileOutputFormats {
...
func convertRun(opts *Options) error {
	...
	// Do the conversion.
	conversion, output, err := doConversion(kittycadClient, kittycad.FileSourceFormat(opts.InputFormat), kittycad.FileOutputFormat(opts.OutputFormat), opts.InputFileBody, opts)
	if err != nil {
...
func doConversion(c *kittycad.Client, srcFormat kittycad.FileSourceFormat, outputFormat kittycad.FileOutputFormat, body []byte, opts *Options) (*kittycad.FileConversion, []byte, error) {
	var b bytes.Buffer
