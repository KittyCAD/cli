...
// NewCmdCompletion creates a new `kittycad completion` command.
func NewCmdCompletion(io *iostreams.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: heredoc.Docf(`
			Generate shell completion scripts for KittyCAD CLI commands.
			...
			Generate a %[1]s_kittycad%[1]s completion script and put it somewhere in your %[1]s$fpath%[1]s:
			...
			Generate a %[1]skittycad.fish%[1]s completion script:
