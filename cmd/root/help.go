...
func rootHelpFunc(cli *cli.CLI, command *cobra.Command, args []string) {
	...
	if longText != "" && command.LocalFlags().Lookup("jq") != nil {
		longText = strings.TrimRight(longText, "\n") +
			"\n\nFor more information about output formatting flags, see `kittycad help formatting`."
	}
...
Read the manual at https://docs.kittycad.io/cli/manual/` + ", or run `kittycad open docs`"})
